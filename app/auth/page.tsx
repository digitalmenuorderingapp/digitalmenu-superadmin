'use client';

import { useState, useEffect } from 'react';
import { useSuperadminAuth, SuperadminAuthProvider } from '@/context/SuperadminAuthContext';
import { toast } from 'react-hot-toast';
import { ShieldCheck, Mail, KeyRound, Loader2, ArrowRight } from 'lucide-react';
import { useRouter } from 'next/navigation';
import { motion, AnimatePresence } from 'framer-motion';

function LoginContent() {
  const [isGoogleLoading, setIsGoogleLoading] = useState(false);
  const { googleSignIn, isAuthenticated, isLoading } = useSuperadminAuth();
  const router = useRouter();

  // GIS Client state
  const [googleAuthClient, setGoogleAuthClient] = useState<any>(null);

  useEffect(() => {
    // Initialize Google Identity Services
    const initializeGis = () => {
      if (typeof window !== 'undefined' && (window as any).google) {
        // Initialize Code Client for Custom UI
        const client = (window as any).google.accounts.oauth2.initCodeClient({
          client_id: process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID,
          scope: 'openid email profile',
          ux_mode: 'popup',
          access_type: 'offline',
          prompt: 'consent',
          callback: async (response: any) => {
            if (response.code) {
              setIsGoogleLoading(true);
              try {
                await googleSignIn(response.code, 'superadmin_browser', 'Superadmin Dashboard');
                router.push('/superadmin/live-stats');
              } catch (error) {
                setIsGoogleLoading(false);
              }
            }
          },
        });
        setGoogleAuthClient(client);
      } else {
        // Retry if script not loaded yet
        setTimeout(initializeGis, 100);
      }
    };

    initializeGis();
  }, []);

  const handleGoogleSignIn = () => {
    if (googleAuthClient) {
      googleAuthClient.requestCode();
    } else {
      toast.error('Google Sign-In is initializing...');
    }
  };

  useEffect(() => {
    if (isAuthenticated && !isLoading) {
      router.replace('/superadmin/live-stats');
    }
  }, [isAuthenticated, isLoading, router]);

  return (
    <div className="min-h-screen bg-[#0f172a] flex items-center justify-center p-4 selection:bg-indigo-500 selection:text-white">
      {/* Background Decorative Elements */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-indigo-600/20 blur-[120px] rounded-full" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-purple-600/20 blur-[120px] rounded-full" />
      </div>

      <div className="w-full max-w-md relative z-10">
        <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-3xl p-8 shadow-2xl shadow-black/50">
          <div className="flex flex-col items-center mb-8">
            <div className="w-16 h-16 bg-gradient-to-tr from-indigo-500 to-purple-500 rounded-2xl flex items-center justify-center shadow-lg shadow-indigo-500/20 mb-4 transform hover:rotate-6 transition-transform">
              <ShieldCheck className="w-10 h-10 text-white" />
            </div>
            <h1 className="text-3xl font-bold text-white tracking-tight">Superadmin</h1>
            <p className="text-slate-400 mt-2">Secure Gateway Login</p>
          </div>

          {/* Premium Custom Google Sign In Button */}
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.2 }}
            className="w-full"
          >
            <button
              onClick={handleGoogleSignIn}
              disabled={isGoogleLoading}
              className="w-full bg-white hover:bg-slate-50 text-slate-800 font-semibold py-3.5 px-4 rounded-xl shadow-lg shadow-black/20 transition-all duration-300 flex items-center justify-center gap-3 active:scale-[0.98] group disabled:opacity-70 disabled:cursor-not-allowed"
              type="button"
            >
              {isGoogleLoading ? (
                <Loader2 className="w-5 h-5 animate-spin text-indigo-600" />
              ) : (
                <>
                  <svg className="w-5 h-5 group-hover:scale-110 transition-transform duration-300" viewBox="0 0 24 24">
                    <path
                      d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                      fill="#4285F4"
                    />
                    <path
                      d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                      fill="#34A853"
                    />
                    <path
                      d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"
                      fill="#FBBC05"
                    />
                    <path
                      d="M12 5.38c1.62 0 3.06.56 4.21 1.66l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 12-4.53z"
                      fill="#EA4335"
                    />
                  </svg>
                  <span className="text-sm tracking-tight">Continue with Google</span>
                </>
              )}
            </button>
          </motion.div>

          <div className="mt-8 pt-8 border-t border-slate-800">
            <p className="text-xs text-center text-slate-500 uppercase tracking-widest leading-loose">
              System monitoring Dashboard v1.0.0<br/>
              Authorized Access Only
            </p>
          </div>
        </div>
      </div>
    </div>
  );
}

export default function Login() {
  return (
    <SuperadminAuthProvider>
      <LoginContent />
    </SuperadminAuthProvider>
  );
}
